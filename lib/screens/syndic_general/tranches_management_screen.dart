...
class _TranchesManagementScreenState extends State<TranchesManagementScreen> {
                    ...
                    child: Column(
                      ...
                      children: [
                        ...
                        FutureBuilder<List<Map<String, dynamic>>>(
                          ...
                          builder: (context, snapshot) {
                            ...
                            return DropdownButtonFormField<int>(
                              ...
                              onChanged: (val) =>
                                  setDialogState(() => selectedSyndicId = val),
                            );
                          },
                        ),
                        ...
                        Row(
                          children: [
                            Expanded(child: _buildCounterField(
                                "Immeubles", nbImmController)),
                            const SizedBox(width: 10),
                            Expanded(child: _buildCounterField(
                                "Appartements", nbAppController)),
                          ],
                        ),
                        ...
                        Row(
                          children: [
                            Expanded(child: _buildCounterField(
                                "Parkings", nbParkController)),
                            const SizedBox(width: 10),
                            Expanded(child: _buildCounterField(
                                "Garages", nbGarController)),
                          ],
                        ),
                        const SizedBox(height: 12),
                        SizedBox(
        ...
        child: Column(
            ...
            children: [
              ...
              FutureBuilder<List<TrancheModel>>(
                ...
                builder: (context, snapshot) =>
                    _buildTopSummary(snapshot.data ?? []),
              ),
  ...
  Widget _buildResponsiveHeader(bool isMobile) {
    if (isMobile) {
      return Column(
        crossAxisAlignment: CrossAxisAlignment.start,
        children: [
          Text("Gestion des Tranches", style: TextStyle(
              fontSize: 24, fontWeight: FontWeight.bold, color: darkGrey)),
          const SizedBox(height: 10),
          _addButton(true),
        ],
      );
    ...
    return Row(
      ...
      children: [
        Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
          Text("Gestion des Tranches", style: TextStyle(
              fontSize: 32, fontWeight: FontWeight.bold, color: darkGrey)),
          const Text("Gérez les tranches de la résidence",
  ...
  Widget _buildTopSummary(List<TrancheModel> list) {
    ...
    return SingleChildScrollView(
      ...
      child: Row(
        children: [
          _kpiItem("Total Tranches", list.length.toString(), Icons.domain,
              Colors.blue),
          const SizedBox(width: 15),
          _kpiItem(
              "Immeubles", totalImm.toString(), Icons.apartment, Colors.green),
          const SizedBox(width: 15),
          _kpiItem("Appartements", totalApp.toString(), Icons.home_work,
              Colors.orange),
        ],
      ),
    );
  ...
  void _showEditTrancheDialog(TrancheModel tranche) {
    // On ne garde que l'ID du syndic actuel
          ...
          StatefulBuilder(
            builder: (context, setDialogState) =>
                AlertDialog(
                  ...
                  backgroundColor: Colors.white,
                  title: Text(
                    "Assigner un Syndic à ${tranche.nom}",
                    style: const TextStyle(fontWeight: FontWeight.bold,
                        fontSize: 18,
                        color: Color(0xFF2C2C2C)),
                  ),
                  content: Column(
                    ...
                    children: [
                      const Text(
                        "Choisissez le nouveau responsable de cette tranche :",
                    ...
                    Row(
                      children: [
                        ...
                        Expanded(
                          child: ElevatedButton(
                            onPressed: () async {
                              // APPEL DE LA MÉTHODE D'ASSIGNATION UNIQUEMENT
                              await _service.assignInterSyndic(
                                  tranche.id, selectedSyndicId);
                              Navigator.pop(context);
                              _loadTranches(); // Rafraîchit l'affichage
