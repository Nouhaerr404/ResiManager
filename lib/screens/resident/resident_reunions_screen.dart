...
class _ResidentReunionsScreenState extends State<ResidentReunionsScreen> {
  ...
  Future<List<Map<String, dynamic>>> _fetchReunionsWithStatus() async {
    final resData = await _db.from('residents').select('appartement_id')
        .eq('id', _residentId!).single();
    final appartData = await _db.from('appartements').select('immeuble_id')
        .eq('id', resData['appartement_id']).single();
    final immData = await _db.from('immeubles').select('tranche_id')
        .eq('id', appartData['immeuble_id']).single();
    final int trancheId = immData['tranche_id'] as int;

    final List reunionsRaw = await _db.from('reunions').select()
        .eq('tranche_id', trancheId).order('date', ascending: true);
    final List participationRaw = await _db.from('reunion_resident')
        .select('reunion_id, confirmation').eq('resident_id', _residentId!);
  ...
  Future<void> _confirm(int reunionId) async {
    ...
    try {
      await _db.from('reunion_resident').upsert({
        'reunion_id': reunionId, 'resident_id': _residentId!,
        'confirmation': choice,
      }, onConflict: 'reunion_id,resident_id');
      setState(() { _statusMap[reunionId] = choice; _pendingMap[reunionId] = null; });
      _snack(choice == 'confirme' ? "✅ Présence confirmée !" : "❌ Absence enregistrée.",
  ...
  Future<void> _cancel(int reunionId) async {
    ...
    try {
      ...
      setState(() { _statusMap[reunionId] = 'en_attente'; _pendingMap[reunionId] = null; });
              ...
              child: isLoading
                ...
                Row(children: [
                  ...
                  Text(
                    ...
                    style: TextStyle(fontWeight: FontWeight.bold,
                        fontSize: 15, color: _color(display)),
                  ),
                ]),
                const SizedBox(height: 16),
                ...
                if (hasConfirmed && !hasPending) ...[
                  Container(
                    ...
                    decoration: BoxDecoration(
                      color: _color(confirmed).withValues(alpha: 0.1),
                      borderRadius: BorderRadius.circular(10),
                    ),
                    child: Row(
                        mainAxisAlignment: MainAxisAlignment.center,
                        children: [
                          ...
                          Text(
                            confirmed == 'confirme'
                                ? "Présence confirmée"
                                : "Absence enregistrée",
                            style: TextStyle(color: _color(confirmed),
  ...
  Widget _choiceBtn({required String label, required IconData icon,
    required Color color, required VoidCallback onTap}) =>
      ElevatedButton.icon(
        ...
        style: ElevatedButton.styleFrom(
          ...
          elevation: 0, side: BorderSide(color: color, width: 1.5),
          shape: RoundedRectangleBorder(
              borderRadius: BorderRadius.circular(12)),
          padding: const EdgeInsets.symmetric(vertical: 14),
        ),
  ...
  Widget _statusBadge(String statut) {
    ...
    return Container(
      ...
      decoration: BoxDecoration(
        color: cfg.$1.withValues(alpha: 0.2),
        borderRadius: BorderRadius.circular(20),
        border: Border.all(color: cfg.$1.withValues(alpha: 0.5)),
      ),
  ...
  Color _color(String s) => s == 'confirme'
      ? const Color(0xFF059669)
      : s == 'absent' ? const Color(0xFFEF4444) : const Color(0xFF6B7280);
  Color _bgColor(String s) => s == 'confirme'
      ? const Color(0xFFF0FDF4)
      : s == 'absent' ? const Color(0xFFFFF1F2) : const Color(0xFFF9FAFB);
  Color _borderColor(String s) => s == 'confirme'
      ? const Color(0xFFBBF7D0)
      : s == 'absent' ? const Color(0xFFFECACA) : const Color(0xFFE5E7EB);
  IconData _icon(String s) => s == 'confirme'
      ? Icons.check_circle_rounded
      : s == 'absent' ? Icons.cancel_rounded : Icons.help_outline_rounded;
}