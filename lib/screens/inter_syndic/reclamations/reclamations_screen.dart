...
class _ReclamationsScreenState extends State<ReclamationsScreen> {
    ...
    return Container(
      ...
      child: Row(children: [
        ...
        Container(
          ...
          child: const Icon(Icons.report_problem_rounded, color: _C.white, size: 20),
        ),
        const SizedBox(width: 10),
        Expanded(
          child: Column(crossAxisAlignment: CrossAxisAlignment.start, mainAxisSize: MainAxisSize.min, children: [
            const Text('Reclamations', style: TextStyle(color: _C.dark, fontWeight: FontWeight.w800, fontSize: 17, letterSpacing: -0.3)),
        ...
        if (_nbEnCours > 0)
          Container(
            ...
            decoration: BoxDecoration(color: _C.amberLight, borderRadius: BorderRadius.circular(20)),
            child: Row(mainAxisSize: MainAxisSize.min, children: [
              const Icon(Icons.circle, size: 7, color: _C.amber),
              const SizedBox(width: 5),
              Text('$_nbEnCours en cours', style: const TextStyle(color: _C.amber, fontSize: 11, fontWeight: FontWeight.w700)),
            ]),
          ),
      ]),
    );
  ...
  Widget _buildStats() {
    return Row(children: [
      _statCard(Icons.hourglass_empty_rounded, '$_nbEnCours', 'En cours', _C.amber, _C.amberLight),
      const SizedBox(width: 10),
      _statCard(Icons.check_circle_outline_rounded, '$_nbResolu', 'Resolus', _C.green, _C.greenLight),
      const SizedBox(width: 10),
      _statCard(Icons.cancel_outlined, '$_nbRejete', 'Rejetes', _C.red, _C.redLight),
    ]);
  }
  ...
  Widget _buildEmpty() {
    return Padding(
      ...
      child: Column(children: [
        Container(width: 64, height: 64, decoration: BoxDecoration(color: _C.coralLight, borderRadius: BorderRadius.circular(18)),
            child: const Icon(Icons.inbox_rounded, color: _C.coral, size: 30)),
        const SizedBox(height: 16),
        const Text('Aucune reclamation', style: TextStyle(color: _C.dark, fontSize: 16, fontWeight: FontWeight.w700)),
        const SizedBox(height: 6),
        Text(
          _filterStatut == 'tous' ? 'Aucune reclamation pour cette tranche' : 'Aucune reclamation avec ce statut',
  ...
  Widget _buildCard(ReclamationModel r) {
    ...
    return Container(
      ...
      decoration: BoxDecoration(
        ...
        border: Border.all(
          color: r.statut == 'en_cours' ? _C.amber.withValues(alpha: 0.5) : _C.divider,
          ...
          child: Row(crossAxisAlignment: CrossAxisAlignment.start, children: [
            ...
            Container(
              ...
              child: Center(
                child: Text(
                  ...
                  style: const TextStyle(color: _C.coral, fontWeight: FontWeight.w800, fontSize: 18),
                ),
              ),
            ),
            ...
            Expanded(child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
              ...
              Row(crossAxisAlignment: CrossAxisAlignment.start, children: [
                Expanded(child: Text(r.titre,
                    style: const TextStyle(fontWeight: FontWeight.w800, fontSize: 14, color: _C.dark, letterSpacing: -0.2))),
                ...
                Container(
                  ...
                  child: Row(mainAxisSize: MainAxisSize.min, children: [
                    ...
                    Text(r.statutEnum.label,
                        style: TextStyle(color: statutColor, fontSize: 10, fontWeight: FontWeight.w700)),
                  ]),
                ),
              ]),
              ...
              // Resident
              Row(children: [
                const Icon(Icons.person_outline_rounded, size: 12, color: _C.textLight),
                const SizedBox(width: 4),
                Text(r.nomComplet.isNotEmpty ? r.nomComplet : 'Resident inconnu',
              ...
              // Description preview
              Text(r.description,
                  style: const TextStyle(color: _C.textMid, fontSize: 12, height: 1.5),
              ...
              Row(children: [
                ...
                const SizedBox(width: 4),
                Text('Soumis le $dateStr', style: const TextStyle(color: _C.textLight, fontSize: 11)),
              ]),
            ])),
          ]),
        ),
        ...
        Container(
          ...
          child: Row(children: [
            ...
            GestureDetector(
              ...
              child: Container(
                ...
                child: const Row(mainAxisSize: MainAxisSize.min, children: [
                  ...
                  SizedBox(width: 5),
                  Text('Details', style: TextStyle(color: _C.blue, fontSize: 11, fontWeight: FontWeight.w700)),
                ]),
              ),
            ),
            const SizedBox(width: 8),
            ...
            GestureDetector(
              ...
              child: Container(
                padding: const EdgeInsets.symmetric(horizontal: 12, vertical: 7),
                decoration: BoxDecoration(color: statutBg, borderRadius: BorderRadius.circular(9)),
                child: Row(mainAxisSize: MainAxisSize.min, children: [
                  Icon(Icons.edit_rounded, size: 13, color: statutColor),
                  const SizedBox(width: 5),
                  Text('Statut', style: TextStyle(color: statutColor, fontSize: 11, fontWeight: FontWeight.w700)),
                ]),
              ),
            ),
            ...
            // Indicateur document
            if (r.documentPath != null && r.documentPath!.isNotEmpty)
              Container(
                padding: const EdgeInsets.symmetric(horizontal: 10, vertical: 7),
                decoration: BoxDecoration(color: _C.greenLight, borderRadius: BorderRadius.circular(9)),
                child: const Row(mainAxisSize: MainAxisSize.min, children: [
                  Icon(Icons.attach_file_rounded, size: 13, color: _C.green),
                  SizedBox(width: 4),
                  Text('Document', style: TextStyle(color: _C.green, fontSize: 11, fontWeight: FontWeight.w700)),
                ]),
              ),
          ]),
        ),
      ...
      builder: (ctx) => Dialog(
        ...
        child: Padding(
          ...
          child: Column(mainAxisSize: MainAxisSize.min, crossAxisAlignment: CrossAxisAlignment.start, children: [
            ...
            // Header dialog
            Row(children: [
              Container(width: 42, height: 42, decoration: BoxDecoration(color: _C.coralLight, borderRadius: BorderRadius.circular(12)),
                  child: const Icon(Icons.report_problem_rounded, color: _C.coral, size: 20)),
              const SizedBox(width: 12),
              Expanded(child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
                const Text('Detail Reclamation', style: TextStyle(fontWeight: FontWeight.w800, fontSize: 16, color: _C.dark)),
                Text('#${r.id}', style: const TextStyle(color: _C.textLight, fontSize: 12)),
              ])),
            ...
            Container(
              ...
              child: Row(children: [
                Icon(_statutIcon(r.statut), size: 14, color: statutColor),
                const SizedBox(width: 8),
                Text(r.statutEnum.label,
                    style: TextStyle(color: statutColor, fontWeight: FontWeight.w700, fontSize: 13)),
              ]),
            ),
            const SizedBox(height: 14),
            ...
            // Resident
            _detailRow('Resident', r.nomComplet.isNotEmpty ? r.nomComplet : 'Inconnu'),
            const SizedBox(height: 10),
            ...
            // Description
            const Text('Description', style: TextStyle(color: _C.textLight, fontSize: 11, fontWeight: FontWeight.w600)),
            const SizedBox(height: 6),
            Container(
              width: double.infinity,
              padding: const EdgeInsets.all(12),
              decoration: BoxDecoration(color: _C.bg, borderRadius: BorderRadius.circular(10)),
              child: Text(r.description, style: const TextStyle(color: _C.dark, fontSize: 13, height: 1.6)),
            ),
            const SizedBox(height: 20),
            ...
            Row(children: [
              Expanded(child: GestureDetector(
                onTap: () { Navigator.pop(ctx); _showStatutDialog(r); },
                child: Container(
                  padding: const EdgeInsets.symmetric(vertical: 13),
                  decoration: BoxDecoration(color: _C.coral, borderRadius: BorderRadius.circular(12)),
                  child: const Text('Changer statut', textAlign: TextAlign.center,
              ...
              GestureDetector(
                ...
                child: Container(
                  ...
                  child: const Text('Fermer', style: TextStyle(color: _C.dark, fontWeight: FontWeight.w700, fontSize: 13)),
                ),
              ),
            ]),
          ]),
        ...
        builder: (ctx, setDialog) => Dialog(
          ...
          child: Padding(
            ...
            child: Column(mainAxisSize: MainAxisSize.min, crossAxisAlignment: CrossAxisAlignment.start, children: [
              ...
              Row(children: [
                Container(width: 42, height: 42, decoration: BoxDecoration(color: _C.coralLight, borderRadius: BorderRadius.circular(12)),
                    child: const Icon(Icons.edit_rounded, color: _C.coral, size: 20)),
                const SizedBox(width: 12),
                Expanded(child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
                  const Text('Changer le statut', style: TextStyle(fontWeight: FontWeight.w800, fontSize: 16, color: _C.dark)),
                  Text(r.titre, style: const TextStyle(color: _C.textLight, fontSize: 12), maxLines: 1, overflow: TextOverflow.ellipsis),
                ])),
                GestureDetector(onTap: () => Navigator.pop(ctx), child: const Icon(Icons.close_rounded, color: _C.textLight)),
              ]),
              const SizedBox(height: 20),
                ...
                return GestureDetector(
                  ...
                  child: AnimatedContainer(
                    ...
                    child: Row(children: [
                      Container(
                        width: 36, height: 36,
                        decoration: BoxDecoration(color: isSelected ? color : _C.bg, borderRadius: BorderRadius.circular(9)),
                        child: Icon(_statutIcon(s.dbValue), size: 17, color: isSelected ? _C.white : _C.textLight),
                      ),
                      const SizedBox(width: 12),
                      Expanded(child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
                        ...
                        Text(_statutDesc(s), style: const TextStyle(color: _C.textLight, fontSize: 11)),
                      ])),
                      if (isSelected)
              ...
              GestureDetector(
                onTap: () async {
                  Navigator.pop(ctx);
                  final err = await _service.updateStatut(r.id, selected);
                  if (!mounted) return;
                  if (err != null) {
  ...
  Widget _detailRow(String label, String value) {
    return Row(crossAxisAlignment: CrossAxisAlignment.start, children: [
      SizedBox(width: 80, child: Text(label, style: const TextStyle(color: _C.textLight, fontSize: 12, fontWeight: FontWeight.w600))),
      const SizedBox(width: 8),
      Expanded(child: Text(value, style: const TextStyle(color: _C.dark, fontSize: 13, fontWeight: FontWeight.w600))),
    ]);
  }
